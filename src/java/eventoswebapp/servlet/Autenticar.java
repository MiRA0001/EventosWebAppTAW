...
public class Autenticar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String mail, pass, status = null, goTo="login.jsp";
        Usuario usuario;
        ...
        if (usuario == null) { 
           ...
           request.setAttribute("status", status);
           goTo = "login.jsp";                       
        } else if (usuario.getRol()== null || usuario.getRol() <1 ||  usuario.getRol() >5  ){
            status = "Su rol no está registrado en el sistema, contacte con un administrador";
           request.setAttribute("status", status);
           goTo = "login.jsp";   
        }
